// Men Products Loader
// Load and display all men's products from localStorage
...
function getMenProducts() {
  ...
  return products.filter(p => {
    const isMen = p.gender === 'Nam' || p.menu === 'nam';
    const isActive = p.status === 'active';
    return isMen && isActive;
  });
}
...
function renderProducts(page = 1) {
  const container = document.getElementById('men-products-grid');
  if (!container) return;
...
function filterBySubCategory(subCategory) {
  if (subCategory === 'Tất cả sản phẩm nam') {
    filteredProducts = [...allMenProducts];
  } else {
    filteredProducts = allMenProducts.filter(p => 
  ...
  if (header) {
    header.textContent = subCategory === 'Tất cả sản phẩm nam' ? 'THỜI TRANG NAM' : subCategory.toUpperCase();
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🛍️ Loading men products...');
  
  allMenProducts = getMenProducts();
  filteredProducts = [...allMenProducts];
  
  console.log(`Found ${allMenProducts.length} men products`);
  
  renderProducts(1);
  updateCartCount();
  
  console.log('✅ Men products loaded successfully');
});
